...
public class TaskManagerUserStepsDefinition {
	...
	
	{
		   ...
		   endUser.i_click_on_WorkGroupTasks();
		   endUser.i_provide_requestid();
		   endUser.i_click_search4();
		   endUser.i_claim_socn(); 
	...
	
	{
		...
		endUser.i_click_on_mytask();
		endUser.i_provide_requestid();
		endUser.i_clear_out_remaining_fields_socn_complete();
		endUser.i_click_search5();
		endUser.i_cliam_socn_complete();
		
		
	}

	@Then("^I see the final status completed for both socn and focn$")
	public void I_see_the_final_status_completed_for_both_socn_and_focn() throws Throwable 
	
	
	{
	    
		endUser.i_click_on_WorkGroupTasks_twice();
